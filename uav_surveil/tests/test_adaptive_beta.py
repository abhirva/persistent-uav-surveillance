...
def test_adaptive_beta_effectiveness():
    ...
    for total_inv in [20, 30, 40, 50]:
        ...
        print(f"   Initial (β=0.2): {fleet_result_initial.n_launch} active + {fleet_result_initial.n_spare} spare")
        
        # Stage-3: Generate routes and schedule
        routes, _ = route_fn(cells, fleet_result_initial.n_launch, config.uav.cruise_speed)
        schedule_summary = schedule_from_config(config, routes)
        
        print(f"   Stage-3B: n_surge={schedule_summary.n_surge}, β_adapt={schedule_summary.β_adapt:.3f}")
        ...
        print(f"   Adaptive (β={schedule_summary.β_adapt:.3f}): {fleet_result_adaptive.n_launch} active + {fleet_result_adaptive.n_spare} spare")
