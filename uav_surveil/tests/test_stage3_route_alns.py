...
def test_alns_loop_time_under_threshold():
    ...
    assert max_loop_time < 280.0, f"Max loop time {max_loop_time:.1f}s exceeds 280s threshold"
    ...
    for route in non_empty_routes:
        assert len(route.cell_sequence) >= 7, f"Route {route.id} has only {len(route.cell_sequence)} cells (min 7)"
    ...
    assert assigned_cells == expected_cells, f"Missing cells: {expected_cells - assigned_cells}"
...
def test_alns_route_balance():
    ...
    if route_sizes:  # Only check if we have routes
        ...
        assert max_size - min_size <= 3, f"Routes too imbalanced: {min_size}-{max_size} cells"
