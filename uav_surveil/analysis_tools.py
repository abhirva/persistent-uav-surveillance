...
def calculate_route_distance(route: Route, cells: Sequence[Cell], depot: Tuple[float, float] = (-500.0, 0.0)) -> float:
    ...
    route_cells = [cell_dict[cell_id] for cell_id in route.cell_sequence if cell_id in cell_dict]
...
def log_route_analysis(routes: List[Route], cells: Sequence[Cell], algorithm_name: str, 
                      cruise_speed: float = 15.0, depot: Tuple[float, float] = (-500.0, 0.0)):
    ...
    for i, route in enumerate(routes):
        ...
        print(f"   Route {i+1:02d}: {cells_count:2d} cells, {distance:6.1f}m, {time_calc:5.1f}s, {efficiency:.4f} cells/m")
    
    print("-" * 80)
    print(f"   Summary: {len(routes)} routes, {total_cells} cells total")
    print(f"   Balance: {min(len(r.cell_sequence) for r in routes)}-{max(len(r.cell_sequence) for r in routes)} cells/route")
    print(f"   Avg route: {total_cells/len(routes):.1f} cells, {total_distance/len(routes):.1f}m, {sum(route_times)/len(routes):.1f}s")
    print(f"   Max loop time: {max(route_times):.1f}s")
    print(f"   Efficiency range: {min(route_efficiencies):.4f} - {max(route_efficiencies):.4f} cells/m")
    print("=" * 80)


def log_cell_coverage_gaps(cells: Sequence[Cell], current_time: float, 
                          threshold: float = 120.0, filename: Optional[str] = None):
    ...
    with open(filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["cell_id", "x", "y", "last_observed", "age", "overdue", "grid_row", "grid_col"])
        
        for cell in cells:
            # Handle both possible attribute names for backwards compatibility
            last_seen = getattr(cell, 'last_seen_ts', None) or getattr(cell, 'last_observed', None)
            ...
            writer.writerow([cell.id, cell.x, cell.y, last_seen, age, overdue, grid_row, grid_col])
    ...
    print(f"   Overdue cells: {len(overdue_cells)} ({len(overdue_cells)/len(cells)*100:.1f}%)")
    print(f"   Coverage threshold: {threshold}s")
    
    if overdue_cells:
        print(f"\n   📍 Overdue Cell Locations:")
        for cell in sorted(overdue_cells, key=lambda c: current_time - (getattr(c, 'last_seen_ts', None) or 0), reverse=True)[:10]:
            last_seen = getattr(cell, 'last_seen_ts', None) or getattr(cell, 'last_observed', None)
            if last_seen is not None and last_seen >= 0:
                age = current_time - last_seen
            else:
    ...
    for region, stats in sorted(gap_analysis.items()):
        if stats["overdue"] > 0:
            ...
            print(f"      {region}: {stats['overdue']}/{stats['total']} overdue ({pct:.1f}%)")
...
def compare_algorithm_performance(results: Dict[str, Dict[str, Any]]):
    ...
    print("=" * 80)
    print(f"{'Algorithm':<12} {'Peak':<8} {'Rolling':<8} {'Min':<8} {'Overdue':<8} {'Loop Time':<10}")
    print("-" * 80)
    ...
    for algo_name, metrics in sorted(results.items()):
        ...
        print(f"{algo_name:<12} {peak:<7.1f}% {rolling:<7.1f}% {min_cov:<7.1f}% {overdue:<7.1f} {loop_time:<9.1f}s")
...
def track_uav_routes(uavs, current_time: float, filename: Optional[str] = None):
    ...
    with open(filename, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["time", "uav_id", "state", "x", "y", "soc", "route_id", "waypoint_idx", 
                        "current_cell", "remaining_cells", "progress_pct"])
        ...
        for uav in uavs:
            ...
            if current_route and current_route.cell_sequence:
                ...
                current_cell = current_route.cell_sequence[waypoint_idx] if waypoint_idx < len(current_route.cell_sequence) else "None"
            ...
            writer.writerow([
                current_time, uav.id, uav.state.value, uav.x, uav.y, uav.soc,
                route_id, waypoint_idx, current_cell, remaining, progress
            ])
...
def analyze_spare_utilization(simulation_logs: List[str], filename: Optional[str] = None):
    ...
    for log_line in simulation_logs:
        if "🚀 Launching spare" in log_line:
            ...
            spare_launches.append({"spare_id": spare_id, "origin_id": origin_id, "event": "launch"})
    ...
    with open(filename, 'w', newline='') as f:
        ...
        for skip in skipped_launches:
            writer.writerow(["skip", skip.get("spare_id", ""), skip.get("origin_id", ""), skip["reason"]])
