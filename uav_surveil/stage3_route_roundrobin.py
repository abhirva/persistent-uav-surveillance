...
def generate_routes_roundrobin(
    ...
    print(
        f"🔄 Round-Robin: Distributing {len(cells)} cells → {n_launch} routes (balanced)"
    )
    ...
    print(
        f"   Target: {target_cells_per_route} cells/route (+{remainder} routes get +1 cell)"
    )
    ...
    # Summary
    longest_loop = (
        max(route.loop_time or 0 for route in route_objects) if route_objects else 0
    )
    summary = {
