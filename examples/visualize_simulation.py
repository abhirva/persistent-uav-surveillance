...
from uav_surveil.analysis_tools import compare_algorithm_performance, log_cell_coverage_gaps
import numpy as np
import time
...
SIM_DURATION = int(config.mission.mission_duration)  # match config mission duration (e.g., 7200s)
...
cell_patches = []
for cell in cells:
    rect = plt.Rectangle((cell.x - CELL_SIZE/2, cell.y - CELL_SIZE/2), CELL_SIZE, CELL_SIZE, color='white', ec='gray', lw=0.5)
    ax.add_patch(rect)
...
cbar = fig.colorbar(mpl.cm.ScalarMappable(norm=norm, cmap=COLORMAP), ax=ax, fraction=0.046, pad=0.04)
...
for i, uav in enumerate(uavs):
    ...
    uav_number = uav.id
    lbl = ax.text(uav.x, uav.y - 8, uav_number, fontsize=8, ha='center', va='top', 
                  fontweight='bold', color='black', 
                  bbox=dict(boxstyle='round,pad=0.2', fc='white', alpha=0.8))
    uav_id_labels.append(lbl)

# HUD text placeholder (updated each frame)
hud_text = ax.text(0.01, 0.99, '', transform=ax.transAxes, va='top', ha='left', fontsize=9,
                   bbox=dict(boxstyle='round,pad=0.3', fc='white', alpha=0.7))
# Move failure banner outside the axes, bottom center of the figure
fail_text = fig.text(0.5, 0.02, '', va='bottom', ha='center', fontsize=8,
                    color='purple', bbox=dict(boxstyle='round,pad=0.2', fc='white', alpha=0.7))
...
ani = animation.FuncAnimation(fig, update, frames=N_FRAMES, interval=FRAME_INTERVAL, blit=True)
...
if hasattr(sim, '_csv_writer') and sim._csv_writer:
    sim._csv_writer.writerow([
        sim.metrics.current_time, sim.metrics.active_uavs, sim.metrics.spare_uavs, 
        sim.metrics.uavs_swapping_battery, round(sim.metrics.coverage_percentage, 1),
        round(sim.metrics.rolling_coverage_avg, 1), sim.metrics.cells_overdue,
        sim.metrics.stl_c2_violations, sim.metrics.c3_alarms, sim.metrics.c3_missed,
        round(sim.metrics.avg_soc, 1), round(sim.metrics.min_soc, 1)
    ])
...
final_coverage_gaps, overdue_cells = log_cell_coverage_gaps(
    sim.cells, sim.metrics.current_time, 
...
rolling_avg = sim.metrics.rolling_coverage_avg if hasattr(sim.metrics, 'rolling_coverage_avg') else 0
