...
def example_3_stage0_integration():
    ...
    print(f"  SoC floor validation: {'✅ Feasible' if is_feasible else '❌ Not feasible'}")
...
def example_4_parameter_studies():
    ...
    for config in configs:
        ...
        print(f"  SoC floor {soc_floor:.1%}: ξ={result.xi_optimal:.3f}, "
...
def example_5_config_manager():
    ...
    study_configs = manager.create_study_configs("baseline", studies, "multi_param_study")
...
def example_7_batch_analysis():
    ...
    print(f"{'Scenario':<12} {'ξ_optimal':<10} {'Feasible':<10} {'Margin (s)':<12} {'Max Grid (m)':<12}")
    print("-" * 60)
    ...
    for scenario_name in scenarios:
        try:
            ...
            print(f"{scenario_name:<12} {result.xi_optimal:<10.3f} {feasible_str:<10} "
